...
public class BandageItem extends ModularItem {
    private final int MAX_USES;
    private final int WARMUP_TICKS;
    private final int HEAL_INTERVAL;
    private PlayerEntity player;

    public BandageItem(Item.Settings settings, ItemRarity rarity, float weight, Integer maxDamage, int warmupTicks, int healInterval) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (stack.getDamage() >= stack.getMaxDamage()) {
            user.sendMessage(Text.literal("绷带已耗尽！"), true);
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (usedTime < WARMUP_TICKS) {
            ...
            if (usedTime % HEAL_INTERVAL == 0) {
                ...
                for (StatusEffectInstance effect : new ArrayList<>(user.getStatusEffects())) {
                    if (shouldTreatEffect(effect.getEffectType())) {
                        treatEffect((PlayerEntity) user, effect);
                        hasTreated = true;
                    }
                }
                if (hasTreated) {
                    stack.damage(1, user, getEquipmentSlot(user.getActiveHand()));
                    // 治疗效果
                    world.playSound(null, user.getX(), user.getY(), user.getZ(),
                            SoundEvents.BLOCK_WOOL_PLACE,
                            SoundCategory.PLAYERS,
                    ...
                    user.stopUsingItem();
                    this.player.sendMessage(Text.literal("身上没有伤口!"), true);
                }
            }
        }
    }

    private boolean shouldTreatEffect(RegistryEntry<StatusEffect> effectEntry) {
        return effectEntry.matchesId(Identifier.of(CrazyKov.MOD_ID, "bleeding"));
    }

    private void treatEffect(PlayerEntity player, StatusEffectInstance effectInstance) {
        RegistryEntry<StatusEffect> effectEntry = effectInstance.getEffectType();
        ...
        this.player.sendMessage(Text.literal("已止血一处伤口!"), true);
    }
    ...
    public void appendTooltip(ItemStack stack, Item.TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.translatable("tooltip.crazykov.bandage.remain", this.MAX_USES - stack.getDamage()).formatted(Formatting.GOLD));
        tooltip.add(Text.translatable("tooltip.crazykov.bandage.preparation", this.WARMUP_TICKS/20.0f).formatted(Formatting.GOLD));
        tooltip.add(Text.translatable("tooltip.crazykov.bandage.interval", (float) this.HEAL_INTERVAL/20.0f).formatted(Formatting.GOLD));
    }
}
